...
export const routes: Routes = [
  { path: 'login', loadComponent: () => import('./auth/login.component').then(m => m.LoginComponent) },
  {
    ...
    children: [
      ...
      {
        path: 'regions',
        loadChildren: () => import('./regions/regions.routes').then(m => m.REGIONS_ROUTES),
        canMatch: [roleGuard(['Admin', 'Editor'])]
      },
      {
        path: 'themes',
        loadChildren: () => import('./themes/themes.routes').then(m => m.THEMES_ROUTES),
        canMatch: [roleGuard(['Admin', 'Editor'])]
      },
      {
        path: 'contacts',
      ...
      {
        path: 'uploads',
        loadChildren: () => import('./uploads/uploads.routes').then(m => m.UPLOADS_ROUTES),
        canMatch: [roleGuard(['Admin', 'Editor'])]
      },
      {
        path: 'users',
